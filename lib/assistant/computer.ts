...
export class PublicBrowserComputer {
  ...
  private async ensurePage(): Promise<Page> {
    ...
    await this.page.setContent(START_PAGE, { waitUntil: 'domcontentloaded' });
    return this.page;
  }
  ...
  private async runNow(actions: ComputerAction[], signal?: AbortSignal): Promise<BrowserFrame> {
      ...
      switch (action.type) {
        ...
        case 'scroll':
          await withModifiers(page, action.keys, async () => {
            await page.mouse.move(numberField(action, 'x', WIDTH / 2), numberField(action, 'y', HEIGHT / 2));
  ...
  run(actions: ComputerAction[], signal?: AbortSignal): Promise<BrowserFrame> {
    return this.serialized(() => this.runNow(actions, signal));
  }

  snapshot(): Promise<BrowserFrame> {
    return this.serialized(async () => this.frame(await this.ensurePage()));
  }

  navigate(rawUrl: string): Promise<BrowserFrame> {
